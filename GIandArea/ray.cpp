...
bool ray::cast(std::vector<object*> &objects, std::vector<light*> &lights, double color[4], int bounces, int lastObject, std::default_random_engine &generator, int primary_ray, int x, int y, int width, double *depthMap, int &primary_objID, vec3 &hit_normal, int &object_type, std::vector<int> &hit_list, int &shadowed, int flat, int light_samples, int indirect_samples, int indirect_bounces) {
	...
	if ((objects[closestObject]->mat->shininess.x > 0 || objects[closestObject]->mat->shininess.y > 0 || objects[closestObject]->mat->shininess.z > 0) && bounces > 0) {
		...
		reflect_ray->cast(objects, lights, reflection_color, bounces - 1, -1, generator, primary_ray, x, y, width, depthMap, primary_objID, hit_normal, object_type, hit_list, shadowed, flat, light_samples, indirect_samples, indirect_bounces);
		delete reflect_ray;
	}
	if ((objects[closestObject]->mat->transparency.x > 0 || objects[closestObject]->mat->transparency.y > 0 || objects[closestObject]->mat->transparency.z > 0) && bounces > 0) {
		...
		if (k < 0) {
			...
			refract_ray->cast(objects, lights, refraction_color, bounces - 1, -1, generator, primary_ray, -1, -1, width, depthMap, primary_objID, hit_normal, object_type, hit_list, shadowed, flat, light_samples, indirect_samples, indirect_bounces);
			delete refract_ray;
		}
		else {
			...
			refract_ray->cast(objects, lights, refraction_color, bounces - 1, closestObject, generator, primary_ray, -1, -1, width, depthMap, primary_objID, hit_normal, object_type, hit_list, shadowed, flat, light_samples, indirect_samples, indirect_bounces);
			delete refract_ray;
		}
	}
