...
class ray {
 public:
  ...
  //bool cast(Scene *theScene, ImageBuffers *imageBuffers, double color[4], int bounces, int lastObject, int primary_ray, int x, int y, int &primary_objID, vec3 &hit_normal, int &object_type, std::vector<int> &hit_list, int &shadowed, int indirect_bounces, double &distance);
  ...
  double connectToLightRay(std::vector<object*> &objects, double distance);
  bool castLightRay(Scene *theScene, double light_ray_intensity[3], double light_ray_hitpoint[3], int bounces, double &cumul_distance);
  ~ray();
  vec3 origin;
  vec3 direction;
