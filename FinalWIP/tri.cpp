...
bool tri::connectingHit(ray *incoming_ray, double &distance) {
	double cosineNE = this->n.dot(incoming_ray->direction);
	if (cosineNE == 0) {
		return false;
	}
...
bool tri::SAT_projection(vec3 &axis, vec3 &center, vec3 &extents, vec3 &box_normal_x, vec3 &box_normal_y, vec3 &box_normal_z, vec3 &vc1, vec3 &vc2, vec3 &vc3) {
	...
	if (std::max(-Max_Double(v1_projection, v2_projection, v3_projection), Min_Double(v1_projection, v2_projection, v3_projection)) > r + 0.0001) {
		return false;
	}
	return true;
}
