...
Scene::Scene(char** argv) {
	...
	while (std::getline(inputFile, lineBuffer)) {
		...
		if (!token.compare("sphere")) {
			...
			tri *t = new tri(vert1, vert2, vert3, tex, lastEmission, m, objectID, m->object_type - 1);
			objects.push_back(t);
			//}
		}
		...
		else if (!token.compare("light_ray_samples")) {
			ss >> light_ray_samples;
		}
		else if (!token.compare("toon_values")) {
			ss >> toon_values[0];
			ss >> toon_values[1];
			ss >> toon_values[2];
		}
		else if (!token.compare("g")) {
			ss >> object_name;
			if (object_name != "default") {
				ID_increment++;
				objectID = ID_increment;
			}
		}
		else {
				//skip line
...
void Scene::readMaterialFile(std::string material_file, std::map<std::string, material *> &materials, std::vector<std::string> &texture_files) {
	...
	while (std::getline(stream, line)) {
		...
		if (!token.compare("newmtl")) {
			...
			// index of refraction (used as object_type)
			ss >> curr_mat->object_type;
		}
		else if (!token.compare("ior")) {
			ss >> curr_mat->ior;
		}
