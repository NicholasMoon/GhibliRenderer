...
int main(int argc, char** argv) {
	...
	cimg_library::CImg<unsigned char> myImage(width, height, 1, 4);
	...
	while (std::getline(inputFile, lineBuffer)) {
		...
		if (!token.compare("sphere")) {
			...
			object *s = new sphere(x, y, z, r, lastColor, m);
			objects.push_back(s);
		}
			...
			ss >> D;
			material *m = new material(shininess, transparency, ior, roughness);
			object *p = new plane(A, B, C, D, lastColor, m);
			objects.push_back(p);
		}
			...
			material *m = new material(shininess, transparency, ior, roughness);
			tri *t = new tri(vert1, vert2, vert3, lastColor, m);
			objects.push_back(t);
		}
		...
		else if (!token.compare("outline_thickness")) {
			ss >> outline_thickness;
		}
		else if (!token.compare("post_process_outline")) {
			post_process_outline = 1;
		}
		else if (!token.compare("outline_samples")) {
			ss >> outline_samples;
		}
	...
	std::uniform_real_distribution<double> distribution2(-outline_thickness, outline_thickness);
			...
			if (spp > 1) {
				for (int sample = 0; sample < spp; sample++) {
					...
					if (shoot) {
						...
						primary_ray->cast(objects, lights, resultColor, bounces, -1, generator, xi, yi, width, depthMap, post_process_outline);
						delete primary_ray;
					}
				}
			}
				...
				if (shoot) {
					primary_ray = new ray(eye.x, eye.y, eye.z, direction.x, direction.y, direction.z);
					if (primary_ray->cast(objects, lights, resultColor, bounces, -1, generator, xi, yi, width, depthMap, post_process_outline)) {
						std::uniform_real_distribution<double> distributionR(0, outline_thickness/depthMap[yi * width + xi]);
						std::uniform_real_distribution<double> distributionTheta(0, 360);
						int edge_vote = 0;
						for (int test_num = 0; test_num < outline_samples; test_num++) {
							double randR = distributionR(generator2);
							double randTheta = distributionTheta(generator2);
							double randx = randR * cos(randTheta * PI / 180);
							double randy = randR * sin(randTheta * PI / 180);
							...
							double edge_test = primary_ray->detect_edge(objects, lights, resultColor, bounces, -1, generator);
							if (edge_test - depthMap[yi * width + xi]  > outline_cutoff) {
								resultColor[0] = 0;
								resultColor[1] = 0;
								resultColor[2] = 0;
								break;
	...
	if (post_process_outline) {
		for (int yi = 1; yi < height - 1; yi++) {
			for (int xi = 1; xi < width - 1; xi++) {
				for (int layer = 0; layer < outline_thickness; layer++) {
