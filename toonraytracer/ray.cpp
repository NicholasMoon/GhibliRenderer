...
bool ray::cast(std::vector<object*> &objects, std::vector<light*> &lights, double color[4], int bounces, int lastObject, std::default_random_engine &generator, int x, int y, int width, double *depthMap, int post_process_outline, int &primary_objID) {
	...
	if ((objects[closestObject]->mat->shininess.x > 0 || objects[closestObject]->mat->shininess.y > 0 || objects[closestObject]->mat->shininess.z > 0) && bounces > 0) {
		...
		reflect_ray->cast(objects, lights, reflection_color, bounces - 1, -1, generator, x, y, width, depthMap, post_process_outline, primary_objID);
		delete reflect_ray;
	}
	if ((objects[closestObject]->mat->transparency.x > 0 || objects[closestObject]->mat->transparency.y > 0 || objects[closestObject]->mat->transparency.z > 0) && bounces > 0) {
		...
		if (k < 0) {
			...
			refract_ray->cast(objects, lights, refraction_color, bounces - 1, -1, generator, -1, -1, width, depthMap, post_process_outline, primary_objID);
			delete refract_ray;
		}
		else {
			...
			refract_ray->cast(objects, lights, refraction_color, bounces - 1, closestObject, generator, -1, -1, width, depthMap, post_process_outline, primary_objID);
			delete refract_ray;
		}
	}
