...
int main(int argc, char** argv) {
	...
	while (std::getline(inputFile, lineBuffer)) {
		...
		if (!token.compare("sphere")) {
			...
			object *s = new sphere(x, y, z, r, lastColor, m, objectID);
			objects.push_back(s);
		}
			...
			if (spp > 1) {
				for (int sample = 0; sample < spp; sample++) {
					...
					if (shoot) {
						...
						primary_ray->cast(objects, lights, resultColor, bounces, -1, generator, xi, yi, width, depthMap, post_process_outline, primary_objID);
						delete primary_ray;
					}
				}
			}
				...
				if (shoot) {
					primary_ray = new ray(eye.x, eye.y, eye.z, direction.x, direction.y, direction.z);
					int primary_objID = -1;
					if (primary_ray->cast(objects, lights, resultColor, bounces, -1, generator, xi, yi, width, depthMap, post_process_outline, primary_objID)) {
						std::uniform_real_distribution<double> distributionTheta(0, 360);
						int edge_rays = 0;
						double step_size = stencil_radius / stencil_rings;
						double radius = 0;
						int test_num = 0;
						for (int ring_radius = stencil_radius; ring_radius > 0; ring_radius -= stencil_rings) {
						...
						double edge_strength = 1 - pow((std::abs(edge_rays - 0.5 * stencil_rings * stencil_ring_samples) / (0.5 * stencil_rings * stencil_ring_samples)),10);
